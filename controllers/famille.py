...
famille = Blueprint('famille', __name__, template_folder='templates')



@famille.route('/famille/accueil/show')
def famille_accueil_show():
    ...
    print("albums_accueil : ", albums_accueil)
    return render_template('Famille/accueil.html', image_accueil=image_accueil, texte_accueil=texte_accueil,
                           albums_accueil=albums_accueil)


@famille.route('/famille/liste/album/show')
def famille_liste_album_show():
    mycursor = get_db().cursor()
    ...
    return render_template('Famille/liste_album.html', info_album=info_album)


@famille.route('/famille/liste/medias/show')
def famille_accueil_medias_show():
    mycursor = get_db().cursor()
    ...
    return render_template('Famille/liste_medias.html', info_medias=info_medias, image=image, video=video,
                           audio=audio)


@famille.route('/famille/info_user/show')
def famille_info_user_show():
    mycursor = get_db().cursor()
    if session['login']:
        ...
        return render_template('Famille/info_user.html', info_user=info_user)
    else:
...
@famille.route('/famille/info_user/edit', methods=["GET"])
def famille_info_user_edit_get():
    mycursor = get_db().cursor()
    if session['login']:
        tuple_select = (session['login'])
        ...
        return render_template('Famille/edit_info_user.html', info_user=info_user)
    else:
        flash(u'Cette action nécessite un compte utilisateur ! Veuillez vous connecter ou bien crée un compte.',
              'alert-warning')
        return redirect('/login')
...
@famille.route('/famille/info_user/edit', methods=["POST"])
...
@famille.route('/famille/suppr_compte')
def famille_suppr_compte():
    mycursor = get_db().cursor()
...
@famille.route('/famille/a_propos')
def famille_a_propos_de_nous():
    mycursor = get_db().cursor()
    sql = "SELECT titre_a_propos AS titre, texte_a_propos AS texte FROM a_propos;"
    mycursor.execute(sql)
    texte = mycursor.fetchall()
    return render_template('Famille/a_propos.html', texte=texte)


@famille.route('/famille/albums/show')
def famille_show_albums():
    mycursor = get_db().cursor()
    ...
    return render_template('Famille/album.html', infos_albums=infos_albums, infos_medias=infos_medias)


@famille.route('/famille/medias/show')
def famille_show_medias():
    mycursor = get_db().cursor()
    ...
    mycursor.execute(sql, tuple_select)
    infos_medias = mycursor.fetchall()
    return render_template('Famille/medias.html', infos_medias=infos_medias)